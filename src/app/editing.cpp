...
bool magnifiable(
	...
	const float dragWidth = size * (EDITING_ITEM_COUNT_PER_LINE - 1) - (13 + style.FramePadding.x * 2) * 2;
	...
	ImGui::PushID("@Sz");
	{
		...
		ImGui::Dummy(ImVec2(xOffset, 0));
		ImGui::SameLine();
		const float posY = ImGui::GetCursorPosY() + 3;
	...
	if (disabled) {
		...
		for (int i = 0; i < 4; ++i) {
			...
			ImGui::PushID(i);
			{
				...
				ImGui::SameLine();
				if (ImGui::ImageButton(theme->iconMinus()->pointer(rnd), ImVec2(13, 13), ImColor(IM_COL32_WHITE))) {
					newVal[i] = Math::max((Int)rptVal - 1, minVal_[i]);
					changed = true;
				}
				ImGui::SameLine();
				ImGui::SetNextItemWidth(dragWidth);
				...
				ImGui::SameLine();
				if (ImGui::ImageButton(theme->iconPlus()->pointer(rnd), ImVec2(13, 13), ImColor(IM_COL32_WHITE))) {
					newVal[i] = Math::min((Int)rptVal + 1, maxVal_[i]);
					changed = true;
				}
			}
			ImGui::PopID();
