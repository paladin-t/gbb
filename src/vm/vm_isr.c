#pragma bank 255

#if defined __SDCC
#else /* __SDCC */
#   error "Not implemented."
#endif /* __SDCC */

#include "vm_isr.h"

BANKREF(VM_ISR)

#if USE_ISR

void isr_vbl(void) NONBANKED NAKED {
#if defined __SDCC && defined NINTENDO
__asm
        ;; This function is reserved for the user-defined VBL ISR. The bank and
        ;; address values are placeholders for later overwriting by the compiler.

        ld e, #0xFF                 ; Can be overwritten by the compiler. E = bank.
        ld hl, #0xFFFF              ; Can be overwritten by the compiler. HL = fn.
        jp ___sdcc_bcall_ehl        ; Call E:HL.
__endasm;
#else /* __SDCC && NINTENDO */
#   error "Not implemented."
#endif /* __SDCC && NINTENDO */
}

void isr_lcd(void) NONBANKED NAKED {
#if defined __SDCC && defined NINTENDO
__asm
        ;; This function is reserved for the user-defined VBL ISR. The bank and
        ;; address values are placeholders for later overwriting by the compiler.

        ld e, #0xFF                 ; Can be overwritten by the compiler. E = bank.
        ld hl, #0xFFFF              ; Can be overwritten by the compiler. HL = fn.
        jp ___sdcc_bcall_ehl        ; Call E:HL.
__endasm;
#else /* __SDCC && NINTENDO */
#   error "Not implemented."
#endif /* __SDCC && NINTENDO */
}

// Installs an overridable VBL ISR defined in named assembly block.
BOOLEAN install_vbl_isr(POINTER THIS, BOOLEAN start, UINT16 * stack_frame) OLDCALL BANKED { // INVOKABLE.
    (void)THIS; (void)start; (void)stack_frame;

    CRITICAL {
        remove_VBL(isr_vbl); // Remove previous installed ISR first.
        add_VBL(isr_vbl); // Install the ISR.
    }

    return TRUE;
}

// Installs the overridable VBL ISR defined in named assembly block.
BOOLEAN uninstall_vbl_isr(POINTER THIS, BOOLEAN start, UINT16 * stack_frame) OLDCALL BANKED { // INVOKABLE.
    (void)THIS; (void)start; (void)stack_frame;

    CRITICAL {
        remove_VBL(isr_vbl);
    }

    return TRUE;
}

// Installs an overridable LCD ISR defined in named assembly block.
BOOLEAN install_lcd_isr(POINTER THIS, BOOLEAN start, UINT16 * stack_frame) OLDCALL BANKED { // INVOKABLE.
    (void)THIS; (void)start; (void)stack_frame;

    CRITICAL {
        remove_LCD(isr_lcd); // Remove previous installed ISR first.
        add_LCD(isr_lcd); // Install the ISR.
    }

    return TRUE;
}

// Installs the overridable LCD ISR defined in named assembly block.
BOOLEAN uninstall_lcd_isr(POINTER THIS, BOOLEAN start, UINT16 * stack_frame) OLDCALL BANKED { // INVOKABLE.
    (void)THIS; (void)start; (void)stack_frame;

    CRITICAL {
        remove_LCD(isr_lcd);
    }

    return TRUE;
}

#endif /* USE_ISR */
