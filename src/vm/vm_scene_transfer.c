...
STATIC void transfer_scene_data(
    ...
    if (w == 1) {
        ...
        for (UINT8 i = 0; i != h; ++i) {
            ...
            set_tiles(dst_x, dst_y + i, w, 1, vram_addr, buf);
        }
        if ((device_type & DEVICE_TYPE_CGB) && attr_bank) {
            VBK_REG = VBK_ATTRIBUTES;
            for (UINT8 i = 0; i != h; ++i) {
    ...
    case DIRECTION_UP:
        for (UINT8 k = 0; k < preload; ++k) {
            transfer_scene_data(
                map_bank, map, attr_bank, attr,
                map_w,
                0, k, 0, DEVICE_SCREEN_BUFFER_HEIGHT - preload + k,
    ...
    case DIRECTION_DOWN:
        for (UINT8 k = 0; k < preload; ++k) {
            transfer_scene_data(
                map_bank, map, attr_bank, attr,
                map_w,
                0, (DEVICE_SCREEN_HEIGHT - preload) + k, 0, DEVICE_SCREEN_BUFFER_HEIGHT - preload + k,
                DEVICE_SCREEN_WIDTH, 1,
                base_tile, current_vram
            );
        }
...
INLINE BOOLEAN transition_scroll(
        ...
        if (horizontal) {
            ...
            if (dir == DIRECTION_UP) {
                transfer_scene_data(
                    map_bank, map, attr_bank, attr,
                    map_w,
                    0, preload + prev_tile, 0, prev_tile,
                    DEVICE_SCREEN_WIDTH, 1,
                    base_tile, current_vram
                );
            } else {
                transfer_scene_data(
                    map_bank, map, attr_bank, attr,
                    map_w,
                    0, prog - 1 - prev_tile, 0, DEVICE_SCREEN_HEIGHT - 1 - prev_tile,
                    DEVICE_SCREEN_WIDTH, 1,
                    base_tile, current_vram
                );
            }
    ...
    if (horizontal) {
        if (dir == DIRECTION_LEFT) { scene_camera_x = (UINT8)  new_offset;  scene_camera_y = 0; }
        else                       { scene_camera_x = (UINT8)(-new_offset); scene_camera_y = 0; }
    } else {
        if (dir == DIRECTION_UP)   { scene_camera_x = 0; scene_camera_y = (UINT8)  new_offset;  }
        else                       { scene_camera_x = 0; scene_camera_y = (UINT8)(-new_offset); }
    }
...
INLINE void transition_normalise(
    ...
    move_bkg(MUL8(SCENE_TRANSITION_SECONDARY_BUFFER_X), MUL8(SCENE_TRANSITION_SECONDARY_BUFFER_Y));
    ...
    scene.width        = map_w;
    scene.height       = DEVICE_SCREEN_HEIGHT;
...
BOOLEAN transition_scene(POINTER THIS, UINT8 start, UINT16 * stack_frame) OLDCALL BANKED {
    ...
    if (start) {
        transition_preload(dir, map_bank, map, attr_bank, attr, map_w, base_tile, state);
        ctx->waitable = TRUE;

        return FALSE;
    }

    if (!transition_scroll(dir, map_bank, map, attr_bank, attr, map_w, base_tile, state)) {
        ctx->waitable = TRUE;

        return FALSE;
    }

    transition_normalise(map_bank, map, attr_bank, attr, map_w, base_tile);
